...
void AccountIntegratedWnd::DrawShadowFrame( RectF const & rect )
{
    ...
    winplus::DrawRoundRectangle( *m_gCached.get(), pen1, RectF( rect.X+1,rect.Y+1,rect.Width,rect.Height ), 10 );
    Pen pen2( Color( 255, 255, 255 ) ); 
    winplus::DrawRoundRectangle( *m_gCached.get(), pen2, rect, 10 );
}

void AccountIntegratedWnd::DrawBackground( RectF const & rect )
{
    winplus::FillRoundRectangle( *m_gCached.get(), SolidBrush( Color( 64, 255, 255, 255 ) ), rect, 10 );
}
