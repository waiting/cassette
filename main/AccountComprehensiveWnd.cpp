...
void AccountComprehensiveWnd::Draw()
{
    ...
    for ( float off = 0; off < _self->rectClient.Height; off += line )
    {
        ...
        g.DrawShadowString( "账户名称", font, &SolidBrush( Color(200,0,0) ), nullptr, rect1, &sf, nullptr );
        g.DrawShadowString( "账号", font, &SolidBrush( Color(200,0,0) ), nullptr, rect2, &sf, nullptr );
