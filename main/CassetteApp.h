...
public:
    ...
    struct SettingsParameters
    {
        ...
        winplus::String databasePath;     // 数据库路径
        winplus::String backupPath;       // 备份路径
        winplus::String wordslibPath;     // 词库路径

        bool isAutoLogin;        // 是否自动登录
        bool isSavePassword;     // 是否保存密码

        winplus::String username;         // 保存的用户名(ec&base64加密)
        winplus::String password;         // 保存的密码(ec&base64加密)
    };
